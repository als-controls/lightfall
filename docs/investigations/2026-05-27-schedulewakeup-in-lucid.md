# `ScheduleWakeup` in LUCID's embedded Claude session — investigation

**Date:** 2026-05-27
**Author:** Claude (Opus 4.7)
**Status:** Investigation only; no behaviour change shipped with this note.
**Companion:** `src/lucid/plugins/agents/engine_tools.py::ncs_wait_for_idle` (Task 1) — the immediate workaround.

## Summary

`ScheduleWakeup` does not currently fire in LUCID's embedded Claude session.
...
*host application* is the harness, and LUCID's host (`PersistentClaudeWorker`,
`src/lucid/claude/_internal/worker.py:182`) does not re-invoke the model when
...
2. `client.receive_response()` (`claude_agent_sdk/client.py:567`) terminates
   immediately after yielding the `ResultMessage`. LUCID's worker mirrors
   that — see `_run_query` in `src/lucid/claude/_internal/worker.py:302` and
   the `break` on `ResultMessage` at line 363.
3. After `_run_query` returns, the worker blocks on `_query_queue.get_nowait()`
   in `_process_queries` (`src/lucid/claude/_internal/worker.py:287`). No
   one is reading the CLI's stdout, and nothing in LUCID is watching the
...
LUCID already wires `PreToolUse` hooks for permission gating
(`src/lucid/claude/permission_manager.py`). Extend this with a wakeup
interceptor:
...
- If the CLI's kairos *does* try to fire and produces an extra synthesized
  ...
  in `<cwd>/.claude/` so LUCID can write it).
...
- A wakeup scheduled before a hard shutdown of LUCID is lost. (Same behaviour
