...
**Goal:** Add a general plan-UI framework to LUCID so plans can embed their own runtime controls as tabs in the Plans panel, and use it to build the `adaptive_experiment` plan that coordinates with Tsuchinoko over NATS.
...
**Tech Stack:** PySide6, bluesky, nats-py (via lucid.ipc), pytest, pytest-qt, pytest-asyncio
...
| File | Responsibility |
|------|----------------|
| `src/lucid/acquire/plan_ui.py` | `plan_with_ui` decorator, `PlanState` base, `PlanUI` base widget |
| `src/lucid/acquire/nats_bridge.py` | `NATSPlanBridge` — NATS subscription → queue bridge |
| `src/lucid/acquire/plans/adaptive.py` | `AdaptivePlanState`, `_state` singleton, `AdaptiveExperimentPanel`, `adaptive_experiment` plan |
...
| File | Changes |
|------|---------|
| `src/lucid/ui/panels/bluesky_panel.py` | Wrap content in QTabWidget, add plan-UI tab lifecycle |
| `src/lucid/acquire/plans/ncs_plans.py` | Register `adaptive_experiment` in `register_ncs_plans` |
...
**Files:**
- Create: `src/lucid/acquire/plan_ui.py`
...
Expected: `ModuleNotFoundError: No module named 'lucid.acquire.plan_ui'`
...
```bash
git add src/lucid/acquire/plan_ui.py tests/acquire/__init__.py tests/acquire/test_plan_ui_framework.py
...
**Files:**
- Create: `src/lucid/acquire/nats_bridge.py`
...
```bash
git add src/lucid/acquire/nats_bridge.py tests/acquire/test_nats_plan_bridge.py
...
**Files:**
- Create: `src/lucid/acquire/plans/adaptive.py`
...
```bash
git add src/lucid/acquire/plans/adaptive.py tests/acquire/test_adaptive_plan.py
...
In `src/lucid/ui/panels/bluesky_panel.py`, modify `_setup_ui` method. Replace the existing body (around lines 108-133) with:
...
In `src/lucid/ui/panels/bluesky_panel.py`, modify `_on_run_requested` to add UI tab creation. Replace the method (around line 365) with:
...
```bash
git add src/lucid/ui/panels/bluesky_panel.py tests/acquire/test_plan_ui_integration.py
...
**Files:**
- Modify: `src/lucid/acquire/plans/ncs_plans.py`

Add the adaptive plan to the default registry so it appears in LUCID's plan selector.

- [ ] **Step 1: Find the plan registration**

Open `src/lucid/acquire/plans/ncs_plans.py` and find the `register_ncs_plans` function (grep for it to locate).

```
cd /c/Users/rp/PycharmProjects/ncs-phase4 && grep -n "register_ncs_plans" src/lucid/acquire/plans/ncs_plans.py
```
...
```
cd /c/Users/rp/PycharmProjects/ncs-phase4 && .venv/Scripts/python -c "from lucid.acquire.plans.registry import create_default_registry; r = create_default_registry(); print(list(r.plan_names))"
```
...
```bash
git add src/lucid/acquire/plans/ncs_plans.py
...
- [ ] **Step 2: Ensure no regressions in broader LUCID tests**
