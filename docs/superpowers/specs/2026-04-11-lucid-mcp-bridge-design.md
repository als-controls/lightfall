# Lightfall MCP Bridge — Design Spec
...
A standalone MCP server that bridges Claude Code to running Lightfall instances via NATS. Claude Code gains the ability to discover, introspect, and control Lightfall through three dynamic tools — without hardcoding any Lightfall-specific actions into the bridge itself.

```
Claude Code  <--stdio-->  MCP Bridge (FastMCP)  <--NATS-->  Lightfall IPCService
```

The bridge is a thin async relay. It does not import Lightfall, PySide6, or any beamline-specific code. It speaks the same NATS JSON protocol that any IPC participant uses.
...
|   +-- lightfall_bridge/
|       +-- __init__.py
|       +-- __main__.py          # Entry point: python -m lightfall_bridge
|       +-- server.py            # FastMCP app, tools, NATS client
+-- skills/
|   +-- setup/
|       +-- SKILL.md             # First-time connection setup guide
+-- tests/
|   +-- conftest.py              # Mock Lightfall NATS responder fixture
...
`--nats-url` is the only required argument. Optional `--default-prefix` sets a default Lightfall instance prefix for convenience.
...
No dependency on Lightfall, PySide6, or any beamline packages.
...
3. Publish to `_lightfall.discover` with the inbox as reply-to.
4. Collect responses for 2 seconds.
5. Unsubscribe and return all responses.

Each Lightfall instance subscribes to the well-known subject `_lightfall.discover` regardless of its topic prefix. This avoids wildcard matching issues with multi-token prefixes (e.g. `als.7011` has two tokens, so `*.meta.actions` wouldn't match it).
...
Invoke an action on a Lightfall instance.
...
```
No response from 'als.7011' -- Lightfall instance may be offline. Timeout: 5s
```

The bridge does not gate unknown actions. If `execute_action` is called with an action not in the cached list, it sends the request anyway. Lightfall will reply with its own error if the action doesn't exist.

## Instance Identity (Lightfall-side change)

Each Lightfall IPCService instance needs a unique identity for discovery:
...
5. New subscription to the well-known `_lightfall.discover` subject. Handler replies with the same payload as `meta.actions` (instance identity + action list).

## Trust & Authentication

The bridge participates in Lightfall's existing trust handshake:
...
1. On first `list_actions` or `execute_action` targeting a new prefix, the bridge sends a request to `{prefix}.auth.request`:
   ...
   ```
2. Lightfall pops a TrustDialog. The operator approves or denies.
...
`"Lightfall instance '{display_name}' denied access. Approve the trust prompt in Lightfall to continue."`

Auth state is session-scoped. No tokens are persisted across bridge restarts, matching Lightfall's trust model.
...
| Lightfall goes away mid-session | `execute_action` gets NATS timeout. Returns timeout message. Cached action metadata remains valid. |
| Malformed JSON from Lightfall | Return raw bytes as text with a warning. Don't crash. |
| Unknown action sent via `execute_action` | Bridge sends it anyway. Lightfall replies with its own error. |
| No prefix given and no default set | Tools that need a prefix return: `"No prefix specified. Use list_instances to discover available Lightfall instances, or set --default-prefix."` Exception: `list_instances` never needs a prefix. |
...
2. Run `list_instances` to discover active Lightfall instances.
3. Confirm the bridge can communicate (trigger auth handshake).
4. Optionally set `--default-prefix` in the MCP config.

Triggered by plugin installation or user saying "connect to Lightfall" / "set up Lightfall bridge".
...
- Full round-trip: bridge tool call -> NATS -> mock Lightfall responder -> bridge response.
...
The mock Lightfall responder is a small async nats-py fixture that subscribes to a prefix and replies to requests. ~30-40 lines.

### Manual testing

Against the local NATS server and a live Lightfall instance. The setup skill facilitates this.
