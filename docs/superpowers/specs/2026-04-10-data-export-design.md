...
Add batch data export to Lightfall, allowing users to select runs in the Data Browser and export them via a headless exporter tool. The exporter runs as an independent process communicating over NATS IPC, keeping heavy processing out of Lightfall's UI thread and enabling multi-workstation deployment.
...
- Run export processing in a separate process to avoid impacting Lightfall's UI performance
...
- Exporter GUI (all parameter collection happens in Lightfall)
...
1. **Data Browser (modified)** — Multi-select + Export button in Lightfall's tiled browser panel
2. **Export Dialog (new)** — Lightfall-side dialog for choosing export type, output directory, and type-specific parameters
3. **Exporter Service (new)** — Headless CLI tool that subscribes to NATS, receives jobs, processes exports, publishes progress

### Communication

All communication uses Lightfall's existing NATS-based IPC infrastructure.

```
Lightfall (workstation B)                    Exporter (workstation B)
┌─────────────────┐                      ┌─────────────────────┐
│ Data Browser     │                      │ lightfall-exporter CLI  │
│  - multi-select  │   NATS request       │                     │
│  - export button │ ──────────────────── │ Subscribe:          │
│                  │   lightfall.export.hostB  │  lightfall.export.hostB │
│ Export Dialog    │                      │                     │
...
│  - progress      │ ◄────────────────── │  - NXsas            │
│  - completion    │   lightfall.export.      │  - NoOp             │
...
Each exporter subscribes to `lightfall.export.<hostname>`. Lightfall publishes to its own hostname's topic. This naturally handles multiple workstations on the same NATS bus — each machine's exporter only receives jobs intended for that machine's local disk.
...
When the user initiates an export, Lightfall:

1. Publishes a ping request to `lightfall.export.<hostname>.ping` (1s timeout)
2. If a reply is received, the exporter is already running — proceed to send the job
3. If no reply, spawn a local exporter: `subprocess.Popen(["lightfall-exporter", "--nats", nats_url])`
4. Retry ping with backoff until the exporter responds (fail after reasonable timeout with error toast)

Lightfall does not track or manage the exporter's lifecycle beyond spawning it. The exporter runs independently until killed.
...
A modal `QDialog` in Lightfall with the following controls:
...
Location: `src/lightfall/ui/dialogs/export_dialog.py`
...
CLI usage:
```
lightfall-exporter --nats nats://localhost:4222
```

### Code Location

```
src/lightfall/exporter/
├── __init__.py
...
- `nats-py` — NATS client (already a Lightfall dependency)
- `tiled[client]` — data access (already a Lightfall dependency)
- `h5py` — HDF5 writing for NXsas
- `numpy` — array operations

All are already Lightfall dependencies or trivial additions.

### Job Message Format

Received via NATS request on `lightfall.export.<hostname>`:
...
Published to `lightfall.export.<hostname>.progress`:
...
Subscribes to `lightfall.export.<hostname>.ping`. Replies with:
...
## Lightfall-Side IPC Integration
...
3. If no response, spawn via `subprocess.Popen(["lightfall-exporter", "--nats", nats_url])`
4. Retry ping with backoff (max ~5 retries, ~5s total)
5. Send job via NATS request to `lightfall.export.<hostname>`
6. On acknowledgment, subscribe to progress events filtered by job ID
7. Show "Export queued" toast

### Progress Display

Subscribe to `lightfall.export.<hostname>.progress`, filter by `job_id`:
