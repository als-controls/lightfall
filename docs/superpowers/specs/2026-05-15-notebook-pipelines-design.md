...
Add post-acquisition (and eventually online) data-processing pipelines to LUCID, implemented as parameterized Jupyter notebooks executed by a separate headless service. Pipelines are distributed as **pip-installable Python packages** that expose a `lucid_pipelines.pipeline` entry point — mirroring LUCID's own plugin model (`AgentPlugin`, `EnginePlugin`, etc.). LUCID submits jobs over NATS carrying a Tiled run UID and a job-scoped Tiled API key; the notebook reads the input run from Tiled, writes derived data back, and emits output run UIDs via Papermill's scrapbook.

The headless executor (`lucid-pipelines`) mirrors the existing `lucid.exporter` service (`src/lucid/exporter/`): per-host NATS topic, request/reply for job submission and health, pub/sub for progress events, on-demand process spawn.
...
- Notebook writes derived data back to Tiled via a thin LUCID-supplied `TiledWriter` wrapper that stamps `parent_run_uid` + `pipeline_provenance` automatically
...
- Pipelines distributed as standard Python packages, discoverable via entry points — uniform with LUCID's other plugin types
...
Once shipped, this primitive also lets LUCID itself move off the access-token refresh treadmill for its own data-browser calls — independent benefit. Implementation lands first, in als-tiled, before any LUCID work depends on it.

## Architecture

### Components

1. **`lucid.pipelines.PipelineClient`** (new, in-process in LUCID) — mints a job-scoped Tiled API key, builds the job message, submits over NATS, tracks progress, revokes the key on completion.
2. **`lucid.pipelines.TriggerManager`** + `Trigger` subclasses (new, in-process in LUCID) — owns the user's configured triggers, hooks `BaseEngine.subscribe()` for engine-driven triggers, routes each fire to `PipelineClient.submit()`.
3. **`lucid-pipelines` service** (new, headless console script) — subscribes to NATS, discovers pipeline plugins via entry points, manages per-pipeline-package venv cache, dispatches Papermill runs in subprocesses, harvests scrapbook outputs, publishes progress.
4. **`PipelinePlugin` base class + `lucid_pipeline` SDK** (new, separately pip-installable) — base class for pipeline authors; `lucid_pipelines.notebook` helpers including the `TiledWriter` wrapper that auto-stamps provenance and `parent_run_uid` on derived runs.
5. **UI surface** in LUCID — Data Browser context-menu action ("Run pipeline…"), pipeline picker / parameter dialog, "Pipeline Jobs" dock panel, "Pipeline Triggers" settings panel.
...
│  ├ RunEndTrigger ─────────┤   │ ─────────►│     lucid.pipeline.<host>        │
│  └ ManualTrigger ─────────┤   │           │                                  │
...
- `lucid.pipeline.<hostname>.list` — request/reply, returns discovered pipelines + parameter schemas
- `lucid.pipeline.<hostname>.refresh` — request/reply, re-scan entry points (after a `pip install`)
- `lucid.pipeline.<hostname>.progress` — pub/sub, progress events per job

There is **no** auto-trigger NATS topic. Triggers fire from inside LUCID via `BaseEngine.subscribe()` and submit jobs through the normal request path.

### On-demand spawn

Same as exporter (`src/lucid/exporter/cli.py`):

1. LUCID pings `lucid.pipeline.<host>.ping` with a 1s timeout.
2. On reply → service running, submit the job.
3. On timeout → `subprocess.Popen(["lucid-pipelines", "--nats", url])`.
4. Retry ping with bounded backoff; surface a toast if startup fails.

Phase 1 is on-demand only. Phase 2+ ships a `lucid-pipelines.service` systemd unit so triggers can fire when LUCID is not running.

## Pipeline plugin model

### `PipelinePlugin` base class

Lives in `lucid_pipelines.plugin`. Parallels `AgentPlugin` / `EnginePlugin` in shape:
...
No automatic GC. Envs accumulate intentionally (rebuild cost is high; cached venvs are not a leak). A `lucid-pipelines cleanup --unused` subcommand removes envs for which no plugin is currently discovered.
...
3. Tiled returns an API key bound to the user's identity. LUCID embeds it in the NATS job message.
...
The key never travels through the executor's logs, the executed notebook (env vars are not serialized into nbformat), or LUCID's persistent state.

### Generalization: `lucid.auth.mint_job_key()`

The mint helper lives in `lucid.auth.mint_job_key(lifetime, scopes, note)` — not in `lucid.pipelines` — because the same primitive solves the same problem for tsuchinoko, future remote-executor offloads, and any other LUCID-dispatched headless workload.
...
- LUCID fetches the input run's `access_blob` at submit time, includes it in the job message.
- Executor exports `LUCID_INPUT_ACCESS_BLOB` (JSON) in the kernel env.
- The `lucid_pipelines.notebook.TiledWriter` wrapper passes the blob into every write, merging the plugin's `output_tags` into the inherited tag list.
...
Manual and auto-triggered jobs both use the user's current session at submit time. For automatic triggers (`RunEndTrigger`, etc.), the trigger fires inside LUCID — which still has the user's session in hand — and the mint happens then, with a TTL covering the expected pipeline runtime.

If the user logs out / closes LUCID before the executor finishes, the API key keeps working until its TTL expires (that's the whole point of moving off the bearer). If LUCID isn't running at all when a binding *would* have fired, the trigger doesn't fire at all (Phase 1 limit — see Non-Goals).

## Trigger model

### `TriggerManager` (LUCID-side, single instance)

Owns the user's configured triggers, persisted in LUCID's settings backend (no YAML file in a registry, no defaults shipped — each beamline configures its own). Subscribes to `BaseEngine.subscribe()` (`src/lucid/acquire/engine/base.py:396`) so triggers operate against any engine (Bluesky, mock, future).
...
Phase 1 triggers hook `BaseEngine.subscribe()`, which only emits docs for runs *executed by the engine*. Pipeline-derived runs are written directly to Tiled by the notebook — they never pass through any engine — so no start/stop doc is emitted to LUCID's subscribers about them. **Therefore no loop is possible in Phase 1; no sentinel tag or filter exclusion is needed.**

(If Phase 2+ adds a "Tiled-poll" trigger that watches Tiled directly for new entries, the loop concern returns and the design will need a sentinel like `metadata.start.lucid_pipeline_output = true` plus a filter that excludes it. Phase 1 doesn't need this.)

### Configuration UI: Pipeline Triggers settings panel

New settings panel (`lucid.ui.panels.pipeline_triggers`) with a table of configured triggers + add/edit/delete dialogs. Each trigger's row shows: type, filter summary, target pipeline, last fired timestamp. Persisted via the existing settings backend.
...
```json
{
  ...
  "input_run_uid": "abcdef…",
  "output_run_uids": ["xyz…"],
  "executed_notebook_path": "/data/lucid-pipelines/runs/abcdef.../<job_id>.ipynb",
...
import os, json
from lucid_pipelines.notebook import TiledWriter, get_input_run, get_provenance
...
LUCID_PIPELINE_PROVENANCE = json.loads(os.environ["LUCID_PIPELINE_PROVENANCE"])

# Convenience: open the input run client
input_run = get_input_run()  # configured tiled client + LUCID_INPUT_RUN_UID
...
### `lucid_pipelines.notebook.TiledWriter` wrapper

Ships in the `lucid-pipelines` SDK. Wraps `bluesky.callbacks.tiled_writer.TiledWriter` and:

- Reads `LUCID_INPUT_RUN_UID` / `LUCID_INPUT_ACCESS_BLOB` / `LUCID_PIPELINE_PROVENANCE` from env.
- Auto-stamps `metadata.start.parent_run_uid = LUCID_INPUT_RUN_UID` on every run written.
- Auto-stamps `metadata.start.pipeline_provenance = LUCID_PIPELINE_PROVENANCE` (`{pipeline_name, pipeline_package, pipeline_package_version, python_executable, env_hash}`).
- Auto-merges `LUCID_INPUT_ACCESS_BLOB.tiled_access_tags + plugin.output_tags` into the write's tags so access inheritance works.
...
- Executor writes the executed notebook to a configurable filesystem path: default `/data/lucid-pipelines/runs/<input_run_uid>/<job_id>.ipynb`. Overridable via `--notebook-store <path>`.
...
Output runs ride through LUCID's existing **Visualization panel** (`src/lucid/visualization`, `BaseVisualization` ABC). Two pathways, both flowing through the same panel:
...
1. `QComboBox` of available pipelines from `lucid.pipeline.<host>.list`.
2. Auto-generated parameter form from the selected plugin's `parameters_schema` (reuse `lucid.plugins.agents.panel_builder` patterns where applicable).
...
- Cancellation is Phase 2+ — add a `lucid.pipeline.<host>.cancel` request later.

### Pipeline Triggers settings panel

Manages configured triggers. Table + add/edit/delete dialogs. Persists via LUCID's settings backend.
...
4. Notebook (in subprocess):
     ...
     b. process
     c. write derived run(s) via lucid_pipelines.notebook.TiledWriter (auto-stamps provenance + access)
...
5. Executor PapermillRunner:
     a. on subprocess exit 0:
        ...
        - write executed.ipynb to /data/lucid-pipelines/runs/R/<job_id>.ipynb
...
| API key revoke fails on completion                      | LUCID cleanup         | Log; key still expires at TTL                                             |
| Executor crash mid-job                                  | LUCID watcher         | After `2 × ping_interval` of no progress + no ping → mark `lost`; user can resubmit |
...
- `tests/pipelines/test_e2e.py` — runs against a real Tiled (`bcgtiled` test instance per `MEMORY.md::reference_als_tiled_deploy`) plus a local NATS:
  ...
  3. Spawn `lucid-pipelines` subprocess pointed at that venv
...
| -------------------- | -------------------------------------- | -------------------------------------------------------- |
| IPC                  | Sirepo internal broker                 | LUCID's existing NATS                                    |
| GUI                  | Browser (Sirepo)                       | Qt desktop (LUCID's existing panels)                     |
...
- **Cancellation.** Add a `lucid.pipeline.<host>.cancel` request taking a `job_id`; executor SIGTERMs the subprocess.
...
- **Systemd unit for the executor** so triggers can fire without LUCID running. Pairs with a fallback identity (service-account API key, output tagged `auto-triggered-by:<service>`).
...
3. **`lucid_pipelines` SDK package boundary.** Single dist with `[project.optional-dependencies]` for executor vs. SDK consumers, or split into multiple dists (`lucid-pipelines-sdk` for pipeline authors, `lucid-pipelines` for the executor)? Lean single-dist with extras for Phase 1.
4. **Notebook-store directory permission model.** Who owns `/data/lucid-pipelines/runs/`? Per-user subdirs, or shared with group-write? Probably ops-decided per deployment; document, don't enforce.
5. **Tsuchinoko shared `TiledWriter` factoring.** After inspecting the `LUCID-refactor` branch, decide whether the wrapper goes in `lucid-pipelines` (and tsuchinoko depends on it) or in a more neutral location (`bluesky-utils`-style package). Defer until comparison is done.

## Implementation skeleton

Staged so each merges independently and the executor is usable from the CLI before LUCID UI lands.
...
**Stage 1 (LUCID core shared primitives):**

2. `lucid.auth.mint_job_key(lifetime, scopes, note)` + tests (also used by tsuchinoko).
3. `lucid.acquire.triggers.{base, manager, run_start, run_end, manual}` + tests (general trigger framework; pipeline-specific routing in Stage 4).
...
7. `lucid.pipelines.PipelineClient` + tests.
