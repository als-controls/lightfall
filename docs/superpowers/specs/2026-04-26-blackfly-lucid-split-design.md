...
**Companion docs:**
- `2026-04-25-lightfall-sdk-native-plugins-design.md` (Spec A — the `AgentPlugin` rails this skill rides on; merged).
...
1. The generic observer-camera abstraction (`CameraBase` ABC + pyqtgraph `CameraImageView`) moves into `lightfall` core under `lightfall.ui.widgets.observers`. This becomes the reusable surface for any future non-ophyd hardware-camera transport.
2. The Blackfly-specific transport stack (GVCP/GVSP/discovery/registers/pixel formats + `BlackflyCamera`) moves into `lightfall-endstation-7011` under `lightfall_endstation_7011.observers.blackfly`. It's beamline-specific FLIR support and does not belong in lightfall core.
3. A new **Blackfly skill** ships from `lightfall-endstation-7011` as a `type_name="agent"` plugin (`BlackflyAgent`). It teaches the embedded Claude agent how to discover Blackfly cameras and wire them into a user `PanelPlugin`, exposing one MCP tool (`discover_blackfly_cameras`) and reusing the existing `panel_builder` agent for file-writing.
...
The term **Device** in lightfall is reserved for **ophyd Devices**. The Blackfly is not ophyd-backed, so its abstraction lives under `observers/`, not `devices/`. The `lightfall.ui.widgets.camera` package (which already exists, holding `OphydImageView` and friends) is the ophyd-flavored peer; `lightfall.ui.widgets.observers` is the new non-ophyd peer.

## 3. Final layout

### 3.1 lightfall (`~/PycharmProjects/ncs/ncs/`)

```
src/lightfall/ui/widgets/observers/
...
### 3.2 lightfall-endstation-7011 (`~/PycharmProjects/ncs/lightfall-endstation-7011/`)

```
src/lightfall_endstation_7011/observers/
  __init__.py
  blackfly/
    __init__.py         # exports BlackflyCamera, discover, DeviceInfo
    camera.py           # BlackflyCamera (subclasses lightfall.ui.widgets.observers.CameraBase),
...
`src/lightfall_endstation_7011/manifest.py` gains one entry:
...
```python
# src/lightfall_endstation_7011/observers/blackfly/skill.py
...
- Wraps `lightfall_endstation_7011.observers.blackfly.discover()`.
...
|---|---|---|
| `test_camera_base.py` | lightfall, `tests/ui/widgets/observers/` | tests the `CameraBase` ABC |
| `test_widgets.py` (renamed `test_image_view.py`) | lightfall, `tests/ui/widgets/observers/` | tests `CameraImageView` |
...
The dependency direction is one-way (endstation imports lightfall; lightfall never imports endstation), so the migration sequences cleanly:

### Step 1 — lightfall first (own branch, own merge)

- Create `src/lightfall/ui/widgets/observers/{__init__.py,camera.py,image_view.py}`.
...
- Lift `CameraImageView` from `blackfly_observer/widgets.py`. Replace `from qtpy import QtCore, QtWidgets` with explicit `from PySide6.QtCore import …` / `from PySide6.QtWidgets import …` (match the form used in `lightfall/ui/widgets/camera/image_view.py`).
- Create `tests/ui/widgets/observers/`, lift `test_camera_base.py` and `test_widgets.py` (rename to `test_image_view.py`).
- Verify under lightfall's venv: `.venv/Scripts/python -m pytest tests/ui/widgets/observers/`.
- Merge.

### Step 2 — endstation second (own branch, own merge)

- Confirm the endstation's workspace install of lightfall picks up step 1's commit.
- Create `src/lightfall_endstation_7011/observers/blackfly/`. Lift `camera.py`, `discovery.py`, `gvcp.py`, `gvcp_transport.py`, `gvsp.py`, `pixel_formats.py`, `registers.py` from `blackfly_observer/src/blackfly_observer/`. In the new `camera.py`, change `from .camera import CameraBase` (which no longer exists) to `from lightfall.ui.widgets.observers import CameraBase`.
...
- Boot lightfall against this endstation, watch the Blackfly skill load in the embedded agent's settings, ask the agent to make a Blackfly panel, watch a live frame stream.
- Merge.

### Step 3 — archive `blackfly_observer`

- Verify both step 1 and step 2 are merged and the integrated lightfall app has been exercised end-to-end (skill loads, `discover_blackfly_cameras` returns the camera, embedded agent creates a panel that streams).
...
- **Skill auto-load.** A typo in the manifest's `import_path` produces a plugin-load error in the lightfall log but does not crash the app. Mitigation: `test_skill.py` instantiates `BlackflyAgent` and asserts the tool list and prompt are non-empty.

**Rollback:** Each step is a separate merge.
- If step 2 misbehaves, revert it; lightfall step 1 is harmless on its own (a couple of unused new modules).
...
- Publishing `lightfall-endstation-7011` to PyPI — dev workflow remains workspace install.

## 10. Open questions

None. All open items from the standing memory (`project_blackfly_lightfall_split_plan.md`) were resolved during the 2026-04-26 brainstorm:
...
| Qt binding | `qtpy` → `PySide6` on the way into lightfall. |
| `bfly-discover` console script | Re-home in `lightfall-endstation-7011` pyproject. |
