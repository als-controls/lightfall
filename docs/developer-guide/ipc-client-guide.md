...
| `commands.device.info`       | `device` (str, required)                                               | `name`, `category`, `device_class`, `pv`                        | `pv` is the device's record/PV prefix (empty when not EPICS-backed); use for client-side CA monitors |
