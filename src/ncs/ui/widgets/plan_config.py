...
for automatic parameter editing.
"""
...
class PlanConfigWidget(QWidget):
    ...
    Uses pyqtgraph's ParameterTree for automatic UI generation from
    function signatures. Provides validation and Run button.
    ...
    def set_plan(self, plan_info: PlanInfo) -> None:
        ...
        self._header_label.setText(plan_info.name)
        ...
        # Convert signature to parameter specs
        param_specs = signature_to_parameters(plan_info.signature, param_docs)
    ...
    def validate(self) -> tuple[bool, list[str]]:
        ...
        for param_info in self._plan.parameters:
            if param_info.required:
                ...
                if child is None:
                    errors.append(f"Missing parameter: {param_info.name}")
                    continue
                value = child.value()
