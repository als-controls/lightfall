...
class MultiStreamBuffer(QObject):
    """Buffer supporting multiple data streams.

    Extends LiveDataBuffer to handle multiple named streams
    (e.g., 'primary', 'baseline', 'monitor').

    Signals:
        stream_data_updated(str, int, dict): Stream name, seq_num, data.
    """

    stream_data_updated = Signal(str, int, dict)
    ...
    def __init__(
        ...
        """Initialize multi-stream buffer.

        Args:
            max_points: Maximum points per stream.
            parent: Optional Qt parent.
        """
        super().__init__(parent)
        self._max_points = max_points
        self._streams: dict[str, LiveDataBuffer] = {}
        self._current_run: RunInfo | None = None
        self._descriptor_to_stream: dict[str, str] = {}  # descriptor UID -> stream name
    ...
    def _handle_descriptor(self, doc: dict[str, Any]) -> None:
        ...
        if stream_name not in self._streams:
            self._streams[stream_name] = LiveDataBuffer(self._max_points, self)

        self._streams[stream_name]._handle_descriptor(doc)
        self._descriptor_to_stream[descriptor_uid] = stream_name

    def _handle_event(self, doc: dict[str, Any]) -> None:
        """Handle event document."""
        descriptor_uid = doc.get("descriptor", "")
        stream_name = self._descriptor_to_stream.get(descriptor_uid, "primary")

        if stream_name in self._streams:
            self._streams[stream_name]._handle_event(doc)
            seq_num = doc.get("seq_num", 0)
            data = doc.get("data", {})
            self.stream_data_updated.emit(stream_name, seq_num, data)

    def _handle_stop(self, doc: dict[str, Any]) -> None:
        """Handle stop document."""
        for buffer in self._streams.values():
            buffer._handle_stop(doc)
        self._current_run = None

    def get_stream(self, name: str = "primary") -> LiveDataBuffer | None:
        ...
        Returns:
            LiveDataBuffer for the stream or None.
        """
    ...
    def clear(self) -> None:
        """Clear all streams."""
        self._streams.clear()
        self._descriptor_to_stream.clear()
        self._current_run = None
